			...
			<tbody>
				...
				<c:forEach var="employee" items="${employees}" varStatus="status">
					<tr class="row${status.count % 2}">
						...
						<td>
							<c:choose>
								<c:when test="${employee.deleteFlag == AttributeConst.DEL_FLAG_TRUE.getIntegrValue()}">
									(削除済み)
								</c:when>
								<c:otherwise>
									<a href="<c:url value='?action=${actEmp}&command=${commandShow}&id=${employee.id}' />">詳細を見る</a>
								</c:otherwise>
							</c:choose>
						</td>
					</tr>
